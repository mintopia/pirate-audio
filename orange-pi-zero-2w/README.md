...
controller, and mainline has no simple I2S driver, so audio runs through
Allwinner's BSP "AHUB" subsystem instead of `hifiberry-dac`.
...
Optional: make the DAC the default ALSA output. Add to `/etc/asound.conf`:
